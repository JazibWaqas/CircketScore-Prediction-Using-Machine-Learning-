"""
Train Models with Mixed Features (Numerical + Categorical)
Handle both numerical and string features for proper cricket prediction
"""
...
def train_with_mixed_features():
    """Train models with both numerical and categorical features"""
    print("TRAINING MODELS WITH MIXED FEATURES (NUMERICAL + CATEGORICAL)")
    print("=" * 80)
    ...
    print(f"Categorical features: {categorical_features[:10]}...")
    ...
    for col in categorical_features:
        ...
        if unseen_mask.any():
            print(f"  - {col}: {unseen_mask.sum()} unseen values, mapping to 0")
            # Map unseen values to 0 (or use a default value)
            test_values = test_values.copy()
            test_values[unseen_mask] = le.classes_[0]  # Use first class as default
    ...
    print(f"\nCombined feature matrix: {X_train_combined.shape}")
    ...
    # Compare models
    print(f"\n" + "="*100)
    print("MIXED FEATURES MODEL COMPARISON - NUMERICAL + CATEGORICAL")
    print("="*100)
    ...
    for model_name, model in models.items():
        with open(f'models/{model_name}_mixed_features.pkl', 'wb') as f:
            pickle.dump(model, f)
        print(f"Model saved: models/{model_name}_mixed_features.pkl")
    
    # Save scaler and label encoders
    with open('models/scaler_mixed_features.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    print(f"Scaler saved: models/scaler_mixed_features.pkl")
    
    with open('models/label_encoders_mixed_features.pkl', 'wb') as f:
        pickle.dump(label_encoders, f)
    print(f"Label encoders saved: models/label_encoders_mixed_features.pkl")
    
    # Save results
    comparison_df.to_csv('models/mixed_features_model_comparison.csv', index=False)
    print(f"Comparison saved: models/mixed_features_model_comparison.csv")
    
    # Honest assessment
    print(f"\n" + "="*80)
    print("HONEST ASSESSMENT - MIXED FEATURES (NUMERICAL + CATEGORICAL)")
    print("="*80)
    ...
    return models, results, comparison_df, best_model, label_encoders

if __name__ == "__main__":
    models, results, comparison, best, encoders = train_with_mixed_features()
